...
import { PackagesReport } from "@/components/reports/packages/packages-report";
import { ArrowLeft } from "lucide-react";
import { Button } from "@/components/ui/button";

interface PackagesReportPageProps {
...
export default async function PackagesReportPage({
  ...
  // Fetch packages report
  const report = await reportService.getPackagesReport(from, to);
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="bg-gradient-to-r from-teal-600 to-cyan-600 bg-clip-text font-nunito text-3xl font-bold text-transparent">
            {t("reports.packages.title")}
          </h1>
          <p className="mt-1 text-sm text-gray-500">
            {t("reports.packages.description")}
          </p>
        </div>
      </div>
