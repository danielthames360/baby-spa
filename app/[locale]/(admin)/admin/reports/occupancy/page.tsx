...
import { OccupancyHeatmap } from "@/components/reports/occupancy/occupancy-heatmap";
...
export default async function OccupancyReportPage({
  ...
  const defaultFrom = new Date(now.getTime() - dayOfWeek * 24 * 60 * 60 * 1000);
  defaultFrom.setHours(0, 0, 0, 0);
  const defaultTo = new Date(defaultFrom.getTime() + 6 * 24 * 60 * 60 * 1000);
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">
            {t("reports.occupancy.description")}
          </p>
        </div>
      </div>
