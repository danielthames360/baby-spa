...
import { BabyCardsReportComponent } from "@/components/reports/baby-cards/baby-cards-report";
import { ArrowLeft } from "lucide-react";
import { Button } from "@/components/ui/button";

interface BabyCardsReportPageProps {
...
export default async function BabyCardsReportPage({
  params,
  searchParams,
}: BabyCardsReportPageProps) {
  ...
  // Fetch baby cards report
  const report = await reportService.getBabyCardsReport(from, to);
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="bg-gradient-to-r from-teal-600 to-cyan-600 bg-clip-text font-nunito text-3xl font-bold text-transparent">
            {t("reports.babyCards.title")}
          </h1>
          <p className="mt-1 text-sm text-gray-500">
            {t("reports.babyCards.description")}
          </p>
        </div>
      </div>
