...
// Simplified intro for public registration page (no next-intl dependency)

const STORAGE_KEY = 'babyspa_registration_intro_shown';
...
export function RegistrationIntro({ tagline, onComplete }: RegistrationIntroProps) {
    ...
    <div className="fixed inset-0 z-[100]">
      <div className={`fixed inset-0 transition-opacity duration-500 ${phase === 'transition' ? 'opacity-0' : 'opacity-100'}`}>
        ...
        <div className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-center transition-all duration-700 z-30 px-4 w-full max-w-sm sm:max-w-md ${showLogo ? 'opacity-100 scale-100' : 'opacity-0 scale-50'}`}>
          ...
          <p className="text-teal-600 mt-1 sm:mt-2 text-base sm:text-lg font-medium">{tagline} 💙</p>
...
// Check if intro should be shown (first visit only for registration)
export function shouldShowRegistrationIntro(): boolean {
